 ...
 */
class com_testsInstallerScript
{
	/**
	...
	function postflight( $type, $parent )
	{
		if ( in_array( $type, array( 'install', 'update' ) ) ) {
			...
			if ( !$this->install_plugins( $type, $parent ) ) {
				JError::raiseWarning( 21, JText::_( 'COM_TESTS_ERROR_INSTALLING_PLUGINS' ) );
			}
		}
	}
	...
	function install_plugins( $type, $parent )
	{
		...
		foreach ( $plg_types as $plg_type ) {
			...
			foreach ( $plugins as $plugin ) {
				...
				if (!$installer->install($p_dir)) {
					...
					JError::raiseWarning( 21, JTEXT::sprintf( 'COM_TESTS_PLUGIN_INSTALL_ERROR',
						$plg_type . '/' . $plugin ) );
					$returns[] = false;
				} else {
					// Package installed sucessfully
					$app->enqueueMessage( JTEXT::sprintf( 'COM_TESTS_PLUGIN_INSTALL_SUCCESS',
						$plg_type . '/' . $plugin ) );
					$returns[] = true;
		...
		if ( !empty( $enable ) ) {
			...
			if ( !$db->query() ) {
				JError::raiseWarning( 1, JText::_('COM_TESTS_ERROR_ENABLING_PLUGINS') );
